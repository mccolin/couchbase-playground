...
try:
  password_auth = PasswordAuthenticator('knights_app', 'password')
  cluster = Cluster('couchbase://127.0.0.1', ClusterOptions(password_auth))
  bucket = cluster.bucket('knights')
...
rows = cluster.query(N1QLQuery('SELECT meta() AS meta, name, email FROM batomic_core WHERE $interest IN interests', interest='Holy Grail'))
